...
function generateLocalFallbackTriage(userMessage) {
  ...
  if (lower.includes('chest') || lower.includes('heart') || lower.includes('breath') || lower.includes('stroke') || lower.includes('unconscious') || lower.includes('bleed')) {
    urgency = 'critical';
    situation_summary = 'User reports symptoms indicating a potential high-severity medical emergency (e.g. chest discomfort, breathing difficulty, or severe distress).';
    recommended_actions = [
  ...
  } else if (lower.includes('fall') || lower.includes('fell') || lower.includes('stand') || lower.includes('slip') || lower.includes('drop')) {
    urgency = 'high';
  ...
  } else if (lower.includes('unsafe') || lower.includes('follow') || lower.includes('stalk') || lower.includes('threat') || lower.includes('scared') || lower.includes('danger')) {
    urgency = 'high';
    ...
    medical_watchouts = [
      'Keep your phone accessible and screen dimmed if necessary.'
    ];
  } else if (lower.includes('allergic') || lower.includes('allergy') || lower.includes('throat') || lower.includes('anaphylaxis') || lower.includes('swelling')) {
    urgency = 'critical';
    ...
    medical_watchouts = [
      'Anaphylaxis can worsen rapidly; do not wait to see if symptoms improve.'
    ];
  }
  ...
  return {
    ...
    disclaimer: 'SafeSense AI provides assistive guidance and does not replace medical diagnosis or emergency 911 services.'
  };
}
...
export async function sendGeminiEmergencyTriage({
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`Serverless API returned status ${response.status}. Using intelligent fallback.`);
      return generateLocalFallbackTriage(prompt);
    }
